...
const PORT = String(28080 + Math.floor(Math.random() * 1000));
...
async function main() {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "cde-docs-signed-export-"));
  ...
  try {
    ...
    const document = assertStatus(await request("POST", "/api/documents", { cookie, body: { name: `signed-${Date.now()}.pdf`, mimeType: "application/pdf", size: fixture.size, dataBase64: fixture.dataBase64, actor: "管理员", conflictMode: "rename" } }), 201, "create signed export document").document;
    const exportResult = assertStatus(await request("POST", `/api/documents/${encodeURIComponent(document.id)}/export`, { cookie, body: {} }), 200, "create signed export");
    if (!/\/exports\/[^?]+\?expires=\d+&signature=[a-f0-9]+/.test(exportResult.downloadUrl || "")) {
      throw new Error(`Export download URL is not signed: ${exportResult.downloadUrl}`);
    }
    const unsignedPath = exportResult.downloadUrl.split("?")[0];
    assertStatus(await request("GET", unsignedPath, { cookie }), 403, "unsigned export download");
    const signedDownload = await request("GET", exportResult.downloadUrl, { cookie });
    if (signedDownload.statusCode !== 200 || !signedDownload.body.startsWith("%PDF")) {
      throw new Error(`Signed export download failed: ${signedDownload.statusCode}: ${signedDownload.body.slice(0, 40)}`);
    }
    console.log(`signed export smoke passed: unsigned blocked and signed URL works on ${BASE_URL}`);
  } catch (error) {
