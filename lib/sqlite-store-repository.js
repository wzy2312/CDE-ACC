...
function readTable(dbPath, tableName) {
  const output = execFileSync("sqlite3", ["-json", dbPath, `SELECT * FROM ${tableName};`], {
    encoding: "utf8",
  }).trim();
...
function readTableColumns(dbPath, tableName) {
  const output = execFileSync("sqlite3", ["-json", dbPath, `PRAGMA table_info(${tableName});`], {
    encoding: "utf8",
  }).trim();
  return output ? JSON.parse(output).map((row) => sqliteValue(row.name)) : [];
}
